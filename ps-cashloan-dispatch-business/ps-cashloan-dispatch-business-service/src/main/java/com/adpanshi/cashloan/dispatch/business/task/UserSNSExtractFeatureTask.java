 ...
 */
@Service
public class UserSNSExtractFeatureTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        logger.info("【用户SNS信息特征抽取 taskInstanceId={}】", taskInstanceId);
        ...
        try {
            ...
            List<Integer> snsVariableIds = (List<Integer>) jsonObject.get(TaskParamConstant.USER_DATA_VARIABLE_ID_LIST);
            if (snsVariableIds == null || snsVariableIds.size() == 0) {
                dispatchRunDomain.finishTaskInstance(taskInstanceId, taskInstanceBo.getParamsJson(), TaskStatus.FAIL);
                return DispatchRunResponseBo.error(StatusCode.FULL_MOXIE_SNS_INFO_FEATURE_FAIL);
            } else {
                //这里是抽取用户SIM信息特征
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_SOCIAL_MOXIESNS, snsVariableIds));
            ...
            logger.error("【用户SNS信息特征抽取异常，异常信息： 】", e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_MOXIE_SNS_INFO_FEATURE_FAIL, e.getMessage());
        }
