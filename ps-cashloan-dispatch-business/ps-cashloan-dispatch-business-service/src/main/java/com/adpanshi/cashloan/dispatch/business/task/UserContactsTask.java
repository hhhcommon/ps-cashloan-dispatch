 ...
 */
@Service
public class UserContactsTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        LOGGER.info("【提交用户通讯录信息 taskInstanceId={}】", taskInstanceId);
        ...
        try {
            ...
            Integer id = userDataDomain.fillCommunication(taskInstatnceBo.getUserDataId(), JSONObject.toJSONString(parseObject.getJSONArray(TaskParamConstant.THIRD_PARTY_METADATA)));
            if (id != null) {
                ...
                dispatchRunDomain.finishTaskInstance(taskInstanceId, taskInstatnceBo.getParamsJson(), TaskStatus.FAIL);
                return DispatchRunResponseBo.error(StatusCode.FULL_CONTACTS_INFO_FAIL);
            }
        } catch (Exception e) {
            LOGGER.error("【提交用户通讯录信息异常，异常信息： 】", e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_CONTACTS_INFO_FAIL, e.getMessage());
        }
    }
}
