 ...
 */
@Service
public class TongdunCreditBodyguardExtractFeatureTask implements Task {

    private static final Logger logger = LoggerFactory.getLogger(TongdunCreditBodyguardExtractFeatureTask.class);
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        logger.info("【同盾信贷保镖信息特征抽取 taskInstanceId={}】",taskInstanceId);
        ...
        try {
            ...
            List<Integer> tdBodyGuardVariableIds = (List<Integer>)jsonObject.get(TaskParamConstant.USER_DATA_VARIABLE_ID_LIST);
            if(tdBodyGuardVariableIds==null || tdBodyGuardVariableIds.size() == 0){
                dispatchRunDomain.finishTaskInstance(taskInstanceId,taskInstanceBo.getParamsJson(), TaskStatus.FAIL);
                return DispatchRunResponseBo.error(StatusCode.FULL_TD_BODYGUARD_INFO_FEATURE_FAIL);
            }else{
                //这里是抽取用户SIM信息特征
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_FRAUD_MOXIESIM, tdBodyGuardVariableIds));
            ...
            logger.error("【同盾信贷保镖信息特征抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_TD_BODYGUARD_INFO_FEATURE_FAIL,e.getMessage());
        }
