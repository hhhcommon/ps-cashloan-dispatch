...
@Service
@Deprecated
public class UserRegisterExtractVariableTask implements Task {
    private static final Logger LOGGER = LoggerFactory.getLogger(UserBaseInfoExtractVariableTask.class);
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        ...
        LOGGER.info("【用户注册信息变量抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            List<UserVariableBo> userVariableBoList = userDataDomain.extractVariable(taskInstanceBo.getUserDataId(), ChannelType.PSAPP,
                    ChannelBizType.APP_USER_BASE_INFO, jsonObject.getInteger(TaskParamConstant.USER_DATA_METADATA_ID));// TODO 业务类型错误
            ...
            if(userVariableBoList.size() > 0){
                ...
                return DispatchRunResponseBo.error(StatusCode.USER_REGISTER_VARIABLE_FAIL);
            }
        } catch (Exception e) {
            LOGGER.error("【用户注册信息变量抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.USER_REGISTER_VARIABLE_FAIL,e.getMessage());
        }
    }
}
