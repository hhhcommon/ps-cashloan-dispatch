 ...
 */
@Service
public class AppApplicationExtractFeatureTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        LOGGER.info("【APP应用特征抽取 taskInstanceId={}】",taskInstanceId);
        ...
        try {
            ...
            if(userVariableIds == null || userVariableIds.size() == 0){
                ...
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_DEVICE_APPLICATION, userVariableIds));
//                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_BASICINFO_FASTTOUCH, userVariableIds));
            ...
            LOGGER.error("【APP应用特征抽取异常，异常信息： 】",e);
