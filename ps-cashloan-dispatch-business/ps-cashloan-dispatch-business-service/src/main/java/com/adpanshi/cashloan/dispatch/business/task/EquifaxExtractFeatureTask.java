 ...
 */
@Service
public class EquifaxExtractFeatureTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        LOGGER.info("【信用报告信息特征抽取 taskInstanceId={}】",taskInstanceId);
        ...
        try {
            ...
            if(jsonObject.get(TaskParamConstant.USER_DATA_VARIABLE_ID_LIST)==null){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_EQUIFAX_CREDITREPORT_INFO_FEATURE_FAIL);
            }
            ...
            if(userVariableIds == null || userVariableIds.size() == 0){
                ...
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_FRAUD_EQUIFAXCREDITREPORT, userVariableIds));
            ...
            LOGGER.error("【信用报告信息特征抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_EQUIFAX_CREDITREPORT_INFO_FEATURE_FAIL,e.getMessage());
        }
