...
/**
 * @author fish_coder
 * @Title: AppCallRecordExtractFeatureTask
 * @ProjectName fenqidai-dubbo
 * @Description: TODO
 * @date 2018/8/1114:52
 */
@Service
public class AppCallRecordExtractFeatureTask implements Task{
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        LOGGER.info("【APP通话记录特征抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            if(jsonObject.get(TaskParamConstant.USER_DATA_VARIABLE_ID_LIST)==null){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_CALLRECORD_FEATURE_FAIL);
            }
            ...
            if(userVariableIds == null || userVariableIds.size() == 0){
                ...
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_SOCIAL_INNER_CROSS_VALIDATION, userVariableIds));
//                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_BASICINFO_FASTTOUCH, userVariableIds));
            ...
            LOGGER.error("【APP通话记录特征抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_CALLRECORD_FEATURE_FAIL,e.getMessage());
        }
    }
}
