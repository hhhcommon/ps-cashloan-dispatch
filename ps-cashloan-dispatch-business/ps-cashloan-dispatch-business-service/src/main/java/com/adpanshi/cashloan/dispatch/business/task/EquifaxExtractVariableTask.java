 ...
 */
@Service
public class EquifaxExtractVariableTask implements Task {

    private static final Logger LOGGER = LoggerFactory.getLogger(EquifaxExtractVariableTask.class);
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        ...
        LOGGER.info("【信用报告信息变量抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            List<UserVariableBo> userVariableBoList = userDataDomain.extractVariable(taskInstanceBo.getUserDataId(), ChannelType.EQUIFAXREPORT,
                    ChannelBizType.EQUIFAX_REPORT_ANALYZE, jsonObject.getInteger(TaskParamConstant.USER_DATA_METADATA_ID));
            ...
            if(userVariableBoList.size() > 0){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_EQUIFAX_CREDITREPORT_INFO_VARIABLE_FAIL);
            }
        } catch (Exception e) {
            LOGGER.error("【信用报告信息变量抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_EQUIFAX_CREDITREPORT_INFO_VARIABLE_FAIL,e.getMessage());
        }
    }
}
