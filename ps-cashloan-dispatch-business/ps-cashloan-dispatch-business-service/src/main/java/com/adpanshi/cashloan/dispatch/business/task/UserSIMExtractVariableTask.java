 ...
 */
@Service
public class UserSIMExtractVariableTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        ...
        logger.info("【用户SIM信息变量抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            List<UserVariableBo> userVariableBoList = userDataDomain.extractVariable(taskInstanceBo.getUserDataId(), ChannelType.MOXIESIMCARD,
                    ChannelBizType.MOXIE_SIM_CARD_INFO, jsonObject.getInteger(TaskParamConstant.USER_DATA_METADATA_ID));
            ...
            if(userVariableBoList.size() > 0){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_MOXIE_SIM_INFO_VARIABLE_FAIL);
            }
        } catch (Exception e) {
            logger.error("【用户SIM信息变量抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_MOXIE_SIM_INFO_VARIABLE_FAIL,e.getMessage());
        }
    }
}
