...
/**
 * @author fish_coder
 * @Title: EmergencyContactExtractVariableTask
 * @ProjectName fenqidai-dubbo
 * @Description: TODO
 * @date 2018/8/1017:35
 */
@Service
public class EmergencyContactExtractVariableTask implements Task {
    private static final Logger LOGGER = LoggerFactory.getLogger(AppApplicationExtractVariableTask.class);
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        ...
        LOGGER.info("【APP紧急联系人变量抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            List<UserVariableBo> userVariableBoList = userDataDomain.extractVariable(taskInstanceBo.getUserDataId(), ChannelType.PSAPP,
                    ChannelBizType.APP_EMERGENCY, jsonObject.getInteger(TaskParamConstant.USER_DATA_METADATA_ID));
            if(userVariableBoList.size() > 0){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_EMERGENCY_INFO_VARIABLE_FAIL);
            }
        } catch (Exception e) {
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_APPLICATION_INFO_VARIABLE_FAIL,e.getMessage());
        }
    }
}
