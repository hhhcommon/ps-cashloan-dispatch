 ...
 */
@Service
public class AppApplicationExtractVariableTask implements Task {
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        ...
        LOGGER.info("【APP应用变量抽取 taskInstanceId={}】", taskInstanceId );
        ...
        try {
            ...
            List<UserVariableBo> userVariableBoList = userDataDomain.extractVariable(taskInstanceBo.getUserDataId(), ChannelType.PSAPP,
                    ChannelBizType.APP_APPLICATION, jsonObject.getInteger(TaskParamConstant.USER_DATA_METADATA_ID));
            if(userVariableBoList.size() > 0){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_APPLICATION_INFO_VARIABLE_FAIL);
            }
        } catch (Exception e) {
            LOGGER.error("【APP应用变量抽取异常，异常信息： 】",e);
