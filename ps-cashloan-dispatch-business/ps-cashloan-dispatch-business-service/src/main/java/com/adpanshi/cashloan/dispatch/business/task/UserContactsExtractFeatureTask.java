 ...
 */
@Service
public class UserContactsExtractFeatureTask implements Task {
    private static final Logger LOGGER = LoggerFactory.getLogger(UserContactsExtractFeatureTask.class);
    ...
    public DispatchRunResponseBo run(Integer taskInstanceId) {
        LOGGER.info("【APP通讯录特征抽取 taskInstanceId={}】",taskInstanceId);
        ...
        try {
            ...
            if(jsonObject.get(TaskParamConstant.USER_DATA_VARIABLE_ID_LIST)==null){
                ...
                return DispatchRunResponseBo.error(StatusCode.FULL_CONTACTS_INFO_FEATURE_FAIL);
            }
            ...
            if(userVariableIds == null || userVariableIds.size() == 0){
                ...
                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_COMMUNICATION_VERFICATION, userVariableIds));
//                applyBoList.add(new ExtractFeatureApplyBo(FeatureType.USER_BASICINFO_FASTTOUCH, userVariableIds));
            ...
            LOGGER.error("【APP通讯录特征抽取异常，异常信息： 】",e);
            ...
            return DispatchRunResponseBo.error(StatusCode.FULL_CONTACTS_INFO_FEATURE_FAIL,e.getMessage());
        }
